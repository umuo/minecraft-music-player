...
test('SOURCE_CONFIG_JSON object form and legacy SOURCE_URL remain compatible', () => {
  ...
  assert.equal(legacy.requestTimeoutMs, 60_000); assert.equal(legacy.scriptTimeoutMs, 60_000)
  assert.throws(() => loadConfig({ SOURCE_URL: 'https://example.test/source.js', REQUEST_TIMEOUT_MS: '60001' }, '/tmp/example'), /1\.\.60000/)
  assert.throws(() => loadConfig({ SOURCE_URL: 'https://example.test/source.js', SCRIPT_TIMEOUT_MS: '60001' }, '/tmp/example'), /1\.\.60000/)
